...
export default function HouseTransits({ transitPositions, natalChart }) {
  ...
  const houseGroups = {};
  Object.entries(transitPositions).forEach(([planet, val]) => {
    if (!PLANET_LABEL[planet]) return;
    // val can be a plain number OR an object { longitude, retrograde, ... }
    const lon = typeof val === 'object' ? val?.longitude : val;
    if (lon == null || isNaN(lon)) return;
    const signIdx = Math.floor(((lon % 360) + 360) % 360 / 30);
    const houseNum = (signIdx - ascSignIdx + 12) % 12 + 1; // always 1-12
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        {Object.entries(houseGroups).sort((a, b) => Number(a[0]) - Number(b[0])).map(([house, planets]) => (
          <div key={house} style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
              <div>
                <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-primary)' }}>{HOUSE_DATA[Number(house)]?.label}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>{HOUSE_DATA[Number(house)]?.focus}</div>
              </div>
            </div>
